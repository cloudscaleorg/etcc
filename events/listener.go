...
//
// snapshot indicates this is the first event
...
const (
	...
	Terminal State = iota
	// opens a watch channel with etcd at the configured prefix and buffers events.
	// this buffer will hold missed events which may occur between Snapshot and Listening
	// states.
	Buffer
	// performs an initial "get" of all k/vs, synthesizing and reducing events, and recording initial revision numbers
	// for each key.
	Snapshot
	// listens to l.eC reducing any buffered events and any subsequent events triggered by the etcd Watch.
	Listening
)
